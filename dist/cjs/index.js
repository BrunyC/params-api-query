...
const getFilter = (filter, params, options) => {
  ...
  return Object.keys(params).map(val => {
    ...
    return {
      ...
      newKey,
      op: parseOperator(op),
      value: parseValue(value, newKey, options)
    };
  }).filter(({
    newKey
  }) => options.blacklist.indexOf(newKey) === -1 && (!options.whitelist || options.whitelist.indexOf(newKey) !== -1)).reduce((result, {
    prefix,
    newKey,
    op,
    value
    ...
    if (Array.isArray(value)) {
      result[newKey][op === '$ne' ? '$nin' : '$in'] = value;
    } else if (op === '$exists') {
      result[newKey][op] = prefix !== '!';
    } else if (op === '$eq') {
      result[newKey] = value;
    } else if (op === '$ne' && typeof value === 'object' && value !== null) {
      result[newKey].$not = value;
    } else {
      result[newKey][op] = value;
    }
